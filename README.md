...
You could build an AI Telegram chatbot powered by the OpenAI GPT model and the Telegram API by GREEN-API. Try our demo, or build your one with [Telegram GPT Bot Library for Python](https://github.com/green-api/telegram-chatgpt-python).
...
Make a copy of the bot chat with ``` git clone https://github.com/green-api/telegram-demo-chatbot-python.git ``` or download the archive [telegram-demo-chatbot-python](https ://github.com/green-api/telegram-demo-chatbot-python).  

The list of necessary libraries is in the requirements.txt file. Run the following command to install them: ``` python -m pip install -r requirements.txt ```. The environment and necessary libraries are installed and ready for running chatbot. You can set up and launch the chatbot on the telegram account.

## Authorization in GREEN-API  

To set up the chatbot on your telegram account you must sign up to [console](https://console.green-api.com/) and register. For new users, [instructions](https://green-api.com/telegram/docs/before-start/) are provided for setting up an account and obtaining the parameters necessary for the chatbot to work, namely:  
```bash
idInstance
apiTokenInstance
```  
...
The [telegram-chatbot-python](https://github.com/green-api/telegram-chatbot-python) library contains a mechanism for changing the instance settings using the [SetSettings](https://green-api.com/telegram/docs/api/account/SetSettings/) method, which is launched when the chatbot is running.
...
Then there will be deletion of the old notifications, another process invoked by the parent library [telegram-chatbot-python](https://github.com/green-api/telegram-chatbot-python), which is essential to make sure chatbot will not process the messages from the already opened chats.
...
## Usage  
After completing the previous steps, you can run the chatbot with your telegram account. It is important to remember that you must be logged in to your [console](https://console.green-api.com/).
...
*6*. Contact 📱
*7*. Location 🌎
*8*. Get image of my avatar👤
*9*. Create a group with the bot 👥
*10*. About PYTHON GREEN API chatbot 🦎
...
```
If you send something other than numbers 1-10, the chatbot will briefly respond:
```
...
The handler receives messages via incoming notifications of type [webhook](https://green-api.com/telegram/docs/api/receiving/notifications-format/incoming-message/Webhook-IncomingMessageReceived/).
...
## Message handling  
The chatbot indicates in its responses, all messages are sent via API. Documentation on sending methods can be found at [Sending methods](https://green-api.com/telegram/docs/api/sending/).

As for receiving messages, messages are read via HTTP API. Documentation on receiving methods can be found at [site](https://green-api.com/telegram/docs/api/receiving/technology-http-api/).

The chatbot uses the [telegram-chatbot-python](https://github.com/green-api/telegram-chatbot-python) library, which already integrates sending and receiving methods, so messages are read automatically, and sending regular text messages is simplified.

For example, the chatbot automatically sends a message to the contact from whom it received a message:
```python
notification.answer(answers_data["select_language"])
```
However, other sending methods can be called directly from the [telegram-api-client-python](https://github.com/green-api/telegram-api-client-python) library. For example, when sending a contact:

```python
notification.api.sending.sendContact(
    chatId=notification.chat,
    contact={
    "phoneContact": notification.sender_phone,
...
Please see file [LICENSE](https://github.com/green-api/telegram-demo-chatbot-python/blob/main/LICENCE).
