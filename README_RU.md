...
- [Documentation in English](https://github.com/green-api/telegram-demo-chatbot-python/blob/main/README.md).  

Демонстрационная версия telegram чатбота основанного на API сервиса [GREEN-API](https://green-api.com). 
При помощи API чатбот отправляет текстовые сообщения, файлы, изображения, музыку, видео, контакты, геолокацию, запрашивает аватар, и создает группу с ботом.
...
```
git clone https://github.com/green-api/telegram-demo-chatbot-python.git
```

Или скачайте архив [telegram-demo-chatbot-python](https://github.com/green-api/telegram-demo-chatbot-python).
...
```  
Среда и необходимые библиотеки установлены. Можно приступить к настройке и запуска чатбота на вашем аккаунте telegram.  
...
В библиотеке [telegram-chatbot-python](https://github.com/green-api/telegram-chatbot-python) прописан механизм изменения настроек инстанса методом [SetSettings](https://green-api.com/telegram/docs/api/account/SetSettings/), который запускается при включении чатбота.
...
После того, как будут применены настройки, произойдет удаление уведомлений о полученных ранее входящих сообщениях. Этот процесс так же прописан в библиотеке [telegram-chatbot-python](https://github.com/green-api/telegram-chatbot-python) и автоматически запускается после изменения настроек.
...
## Использование  
Если предыдущие шаги были выполнены, то на вашем аккаунте telegram должен работать чатбот. Важно помнить, что вы должны быть авторизованы в [личном кабинете](https://console.green-api.com/).
...
*10*. О PYTHON GREEN API чат-боте 🦎
...
```
Если отправить что-то помимо чисел 1-10, то чатбот лаконично ответит:
```
...
Обработчик получает сообщения через входящие уведомления типа [webhook](https://green-api.com/telegram/docs/api/receiving/notifications-format/incoming-message/Webhook-IncomingMessageReceived/).
...
Как и указывает чатбот в ответах, все сообщения отправлены через API. Документацию по методам отправки сообщений можно найти на сайте в разделе [Методы отправки](https://green-api.com/telegram/docs/api/sending/).

Что касается получения сообщений, то сообщения вычитываются через HTTP API. Документацию по методам получения сообщений можно найти на [сайте](https://green-api.com/telegram/docs/api/receiving/technology-http-api/).

Чатбот использует библиотеку [telegram-chatbot-python](https://github.com/green-api/telegram-chatbot-python), где уже интегрированы методы отправки и получения сообщений, поэтому сообщения вычитываются автоматически, а отправка обычных текстовых сообщений упрощена.

Например, чатбот автоматически отправляет сообщение контакту, от которого получил сообщение:
```python
notification.answer(answers_data["select_language"])
```
Однако другие методы отправки можно вызвать напрямую из библиотеки [telegram-api-client-python](https://github.com/green-api/telegram-api-client-python). Как, например, при отправке контакта:
```python
...
[LICENSE](https://github.com/green-api/telegram-demo-chatbot-python/blob/main/LICENCE).  
